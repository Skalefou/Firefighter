...
enum {SELECTION_SOUND, DEAD_SOUND, PAUSE_SOUND};
...
public:
    ...
    void pauseMusic(const int id);
    float getVolumeMusic() const;
    float getVolumeSound() const;
    void FreeMenuMusic();
private:
    std::vector <sf::Music*> m_music;
    const std::vector <std::string> m_nameMusicFile {"data/mainTheme.ogg", "data/game.ogg", "data/gameOver.ogg"}, m_nameSoundFile {"data/selectionSound.ogg", "data/dead.ogg", "data/pauseMenu.ogg"};
